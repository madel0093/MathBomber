...
package com.freeactionscript
{
	...
	public class SimplePhysics
	{
		 ...
		 */
		public function createBalls(numberOfBalls:Number,SR:Stage,current_level:Number):void
		{
			trace("Create balls: " + numberOfBalls);
			for (var i:int = 0; i < numberOfBalls; i++)
			{
				createBall(SR,current_level);
			}
		}
		 ...
		 */
		public function createBall(SR:Stage,current_level:Number):void
		{
			// Create new ball
			// precalculate ball properties
			var newX:Number = Math.random() * _maxX;
			var newY:Number = Math.random() * _maxY;
			var newSpeed:Number = 4;
			var newRadius:Number = 35;
			var newRotation:Number = Math.floor(Math.random() * 360);
			
			// Ball(x, y, radius, rotation, speed, mass) 
			var ball:Ball = new Ball(newX, newY, newRadius, newRotation, newSpeed, newRadius,SR,_ballArray.length,this,current_level);
